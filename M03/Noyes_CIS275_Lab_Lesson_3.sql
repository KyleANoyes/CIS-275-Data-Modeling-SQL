...
USE		Examples
SELECT	CONVERT(CHAR(14), CustomerLast) AS "Customer Name",
		CONVERT(CHAR(20), CustCity) AS "Customer City",
		CONVERT(CHAR(30), VendorName) AS "Vendor Name",
		CONVERT(CHAR(20), VendorCity) AS "Vendor City"
FROM	Customers JOIN Vendors ON CustCity = VendorCity
ORDER BY CustCity,
		 CustomerLast,
		 VendorName;
...
USE		Examples
SELECT	CONVERT(CHAR(14), CustomerLast) AS "Customer Name",
		CONVERT(CHAR(20), CustState) AS "Customer State",
		CONVERT(CHAR(30), VendorName) AS "Vendor Name",
		CONVERT(CHAR(20), VendorState) AS "Vendor State"
FROM	Customers JOIN Vendors ON CustState = VendorState
WHERE	CustState NOT IN ('IL', 'MA')
...
USE		CISDB
SELECT	CONVERT(CHAR(11), F_INVOICE.InvoiceNbr) AS "Invoice #",
		CONVERT(CHAR(10), F_INVOICE.InvoiceDt, 126) AS "Date",
		CONVERT(CHAR(10), FORMAT(F_INVOICE.TotalPrice, '$#.00')) AS "Total Price",
		CONVERT(CHAR(11), F_EMPLOYEE.EmpID) AS "Employee ID",
		CONVERT(CHAR(21), F_EMPLOYEE.Name) AS "Employee Name",
		CONVERT(CHAR(20), F_EMPLOYEE.Title) AS "Employee Title"
FROM	F_EMPLOYEE INNER JOIN F_INVOICE ON F_EMPLOYEE.EmpID = F_INVOICE.FK_EmpID
WHERE	F_INVOICE.InvoiceDt IN ('2002-04-12') AND
		F_EMPLOYEE.Name IN ('Tim Brown');
...
USE		Examples
SELECT  InvoiceID AS "Invoice ID", 
        CAST(InvoiceNumber AS varchar) AS "Invoice Number",
		InvoiceTotal AS "Invoice Total"
FROM    Invoices
WHERE   InvoiceTotal > 50
   UNION
...
/* 
	I can not find an issue with the below query. While it doesn't match the
	expected output, I can't find matching data when using SELECT * and
	manually searching the returned data.

	I'm going to roll with this for now as I feel my solution to this 
	problem statement is in line with the intended learning outcome.
*/

USE		IMDB
SELECT	CONVERT(CHAR(50), title_basics.primaryTitle) AS "Primary Title",
		CONVERT(CHAR(8), seasonNumber) AS "Season#",
		CONVERT(CHAR(15), numVotes) AS "Number of Votes"
FROM	title_basics
		JOIN title_episode ON title_basics.tconst = title_episode.parentTconst
		JOIN title_ratings ON title_basics.tconst = title_ratings.tconst
WHERE	primaryTitle LIKE ('%Queen%') AND
		title_episode.seasonNumber IS NOT NULL AND
		title_ratings.numVotes BETWEEN 5 AND 7
...
USE		IMDB
SELECT	CONVERT(CHAR(31), title_basics.primaryTitle) AS "Title",
		CONVERT(CHAR(12), title_basics.titleType) AS "Type",
		CONVERT(CHAR(11), title_basics.startYear) AS "Year",
		CONVERT(CHAR(20), name_basics.primaryName) AS "Primary Name",
		CONVERT(CHAR(10), title_genre.genre) AS "Genre"
FROM	title_basics
		JOIN title_genre ON title_genre.tconst = title_basics.tconst
		JOIN title_directors ON title_directors.tconst = title_basics.tconst
		JOIN name_basics ON name_basics.nconst = title_directors.nconst
WHERE	title_genre.genre IN ('Drama') AND
		name_basics.primaryName IN ('Akira Kurosawa')
...
USE		IMDB
SELECT	CONVERT(CHAR(30), title_basics.primaryTitle) AS "Title",
		CONVERT(CHAR(12), title_basics.titleType) AS "Type",
		CONVERT(CHAR(11), title_basics.runtimeMinutes) AS "Run Time",
		CONVERT(CHAR(12), title_basics.startYear) AS "Year",
		CONVERT(CHAR(15), name_basics.primaryName) AS "Direction/Writer"
FROM	title_basics
		JOIN title_directors ON title_directors.tconst = title_basics.tconst
		JOIN title_writers ON title_writers.tconst = title_basics.tconst
		JOIN name_basics ON name_basics.nconst = title_writers.nconst
WHERE	title_writers.nconst = title_directors.nconst AND
		name_basics.primaryName IN ('Al Pacino')
...
USE		IMDB
SELECT	CONVERT(CHAR(20), name_basics.primaryName) AS "Primary Name",
		CONVERT(CHAR(30), title_basics.primaryTitle) AS "Series Name",
		CONVERT(CHAR(12), title_basics.titleType) AS "Type",
		CONVERT(CHAR(06), title_ratings.averageRating) AS "Rating",
		CONVERT(CHAR(11), title_basics.startYear) AS "Year"
FROM	title_basics
		JOIN title_ratings ON title_ratings.tconst = title_basics.tconst
		JOIN title_directors ON title_directors.tconst = title_basics.tconst
		JOIN name_basics ON name_basics.nconst = title_directors.nconst
WHERE	name_basics.primaryName IN ('Wes Craven') AND
		title_basics.titleType IN ('tvSeries')
...
/*
	This is another instance of where I can not find a fault in
	my query and feel that the below is correct. I'm going to
	continue with this as I feel my query meets the intended 
...
USE		MyGuitarShop
SELECT	TOP 10
		CONVERT(CHAR(11), Orders.OrderID) AS "OrderID",
		CONVERT(CHAR(12), Products.ListPrice) AS "Item Price",
		CONVERT(CHAR(21), Orders.OrderDate) AS "Order Date"
FROM	Orders
...
/* 
	Find all instances where the vendor ID matches invoice totals between
	500 and 1250 while vendor ID is between 0 and 500
*/

USE		AP
SELECT	CONVERT(CHAR(10), VendorID) AS "Vendor ID"
FROM	Invoices
WHERE	Invoices.InvoiceTotal BETWEEN 500 AND 1250
INTERSECT
SELECT	VendorID
FROM	Vendors
WHERE	Vendors.VendorID BETWEEN 0 AND 500
